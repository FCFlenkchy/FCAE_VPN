...
impl Policy {
    pub fn from_env() -> Result<Option<Self>> {
        ...
        Self::parse(spec).map(Some).ok_or_else(|| AetherError::Other(
            "invalid AETHER_EXIT_LOC: use country codes such as DE,SE or !IR,AZ,RU".into(),
        ))
    }
    ...
    pub fn parse(raw: &str) -> Option<Self> {
        ...
        if codes.is_empty()
            || codes.iter().any(|c| c.len() != 2 || !c.bytes().all(|ch| ch.is_ascii_alphabetic()))
        {
            return None;
        }
    ...
    pub fn accepts(&self, loc: &str) -> bool {
        ...
        if code.len() != 2
            || !code.bytes().all(|ch| ch.is_ascii_alphabetic())
...
pub fn parse_trace(body: &str) -> Option<String> {
    field(body, "loc")
        ...
        .filter(|code| code.len() == 2 && code.bytes().all(|ch| ch.is_ascii_alphabetic()))
}
...
pub async fn watch(stack: &StackHandle, policy: &Policy) -> AetherError {
    loop {
        ...
        match lookup(stack).await {
            ...
            Err(e) => return AetherError::Other(format!("exit location could not be revalidated: {e}")),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn an_unknown_or_malformed_country_is_never_accepted() {
        let policy = Policy::parse("!IR").unwrap();
        for country in ["", "?", "00", "DEU"] {
            assert!(!policy.accepts(country));
        }
        assert!(parse_trace("loc=00\n").is_none());
        assert!(Policy::parse("DE,invalid").is_none());
        assert!(Policy::parse("!IR,").is_none());
    }
