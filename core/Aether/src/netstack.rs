...
pub struct NetStack {
    /// Whether this stack's traffic counts toward the global RX/TX totals.
    ///
    /// Only the user-facing tunnel should count. Internal/plumbing stacks
    /// (e.g. the outer stack of a warp-in-warp chain) carry the same bytes a
    /// second time, so counting them would double every figure in the UI.
    count_stats: bool,
    iface: Interface,
    device: StackDevice,
...
/// Like [`spawn`], but the traffic does NOT count toward the global RX/TX
/// totals.
///
/// Used for the outer stack of a warp-in-warp chain: those bytes are the same
/// payload the inner stack already counted, just wrapped again, so metering
/// both would double every number the UI shows.
