...
impl Adapter {
    pub(super) fn start(upstream: SocketAddr, resolvers: Vec<Ipv4Addr>) -> io::Result<Self> {
        ...
        let thread = std::thread::Builder::new().name("zeptun-socks5p".into()).spawn(move || {
        ...
        tokio::select! {
            ...
            accepted = listener.accept() => {
                let (client, peer) = match accepted {
                    Ok(value) => value,
                    Err(e) => { log::error!("[zeptun socks5p] accept failed: {e}"); break; }
                };
