...
//!   so the app ships upstream's `hev-socks5-tunnel.exe` built by the
//!   `build-hev-windows` CI job.
    ...
    if target.os == fcae_build::target::Os::Windows && !msys_target() {
        ...
        panic!(
            "the hev-socks5-tunnel engine has no native Windows port (its Windows backend is \
             ...
             Build it with the `build-hev-windows` job and point FCAE_HEV_SIDECAR_EXE at \
